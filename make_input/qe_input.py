...
from raw_info import Ang2au,z2eleminfo,dont_print_wyck_dict
...
def makeQEInput_new(crystal,sites_z,symprec=1e-5,
    ...
    new_crystal,cell_par, positions_data = frame2qe_format_new(crystal,sites_z,sym_data)
    ...
    kwargs = dict(crystal=new_crystal,sym_data=sym_data, cell_par=cell_par, positions_data=positions_data,
...
def makeQEInput_new(crystal, sites_z,cell_par,positions_data,sym_data,ibrav0=False,
                    rhocutoff=20 * 4, wfccutoff=20,
                   calculation_type='"scf"', restart_mode='"from_scratch"',
                   smearing=1e-2,  inequivalent_pos=None,
                   etot_conv_thr=1e-4, forc_conv_thr=1e-3, nstep=150,
                   scf_conv_thr=1e-6, print_forces=True, print_stress=False,
    ...
    if spaceGroupIdx in dont_print_wyck_dict.keys() and ibrav != 0:
        for it in range(len(positions_data['wickoffs'])):
            if positions_data['wickoffs'][it] in dont_print_wyck_dict[spaceGroupIdx]:
                positions_data['wickoffs'][it] = ''
